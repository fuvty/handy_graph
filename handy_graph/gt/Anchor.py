...
def GenVMap(subgraph: nx.Graph, graph: nx.Graph) -> list[map]:
    ...
    maps = []
    for vmap in vmaps:
        map = dict((zip(list(subgraph.get_vertices()),[vmap[v] for v in subgraph.vertices()])))
        maps.append(map)
    ...
    for i in tqdm(range(matrix_shape[0])):
        for j in range(matrix_shape[1]):
            if lessSMatrix[i,j] == 1:
                subgraph_anchor, graph_anchor = GenAnchor(motifs[i], motifs[j], method=method)
                anchorMarix[i,j,0] = subgraph_anchor
                anchorMarix[i,j,1] = graph_anchor

    np.save('playground/vars/'+str(data_range)+'/anchorMatrix.'+method, anchorMarix)
